...
- 论文目录现有 14 篇：新增 Kim2023 跳跃与栖停机制，结合共享阅读聊天与论文原文，整理被动收紧、能量曲线、角度关系和参数扫描；Figure 3–5 保留原图标注与 CC BY 4.0 来源。
- 原有 13 篇补充机制说明、带条件的参数表、实验建议、适用边界和关联阅读。`knowledge/papers/connections.html` 按接触、释放、机制切换和动态栖停组织阅读路线；搜索、首页数量和站点地图同步更新。
- 资料依据 myvault 的论文摘要/概念页及 paperreading 的精读记录，重要解释按原文核对。论文结果、项目假设与当前实验进度分开；共享聊天原始文本不放入发布目录。
